...
from src.resources.mapper import transaction, several_transactions

@pytest.mark.parametrize("conditions", [
    {
        "transaction": transaction("ORDERED", 80, 70),
        "testrail_case_id": 11342
    },
    {
        "transaction": transaction("SHIPPED", 30, 20),
        "testrail_case_id": 11343
    },
    {

        "transaction": transaction("DELIVERED", 90, 80),
        "testrail_case_id": 11344
    },
    {
        "transaction": transaction("DO_NOT_REORDER", 50, 40),
...
def test_sales_orders_status_v2_update_status_and_quantity(srx_integration_api, sync_order_location_preset, conditions, delete_shipto):
    ...
    items_list = [
            {
                "transactionType": param.status,
                "id": f"{transaction_id}",
                "release": 1,
                "items": [
                    {
                        "dsku": sku,
                        "quantityShipped": param.quantity_shipped,
                        "quantityOrdered": param.quantity_ordered
                    }
                ]
            }
        ]
...
def test_sales_orders_status_v2_update_several_transactions(srx_integration_api, sync_order_location_preset, conditions, delete_shipto):
    ...
    items_list = [
            {
                "transactionType": init.status_1,
                "id": f"{transaction_id}",
                "release": 1,
                "items": [
                    {
                        "dsku": sku,
                        "quantityShipped": init.quantity_shipped_1,
                        "quantityOrdered": init.quantity_ordered_1
                    }
                ]
            },
            {
                "transactionType": init.status_2,
                "id": f"{transaction_id}",
                "release": 2,
                "items": [
                    {
                        "dsku": sku,
                        "quantityShipped": init.quantity_shipped_2,
                        "quantityOrdered": init.quantity_ordered_2
                    }
                ]
            }
        ]
