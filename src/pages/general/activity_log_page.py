...
class ActivityLogPage(BasePage):
    ...
    def get_main_activity_log_row(self, row_number):
        ...
        row_objects = self.driver.find_elements_by_xpath(f"(({Locator.xpath_by_count(Locator.xpath_table, 1)}{Locator.xpath_table_row})[{row_number}]{Locator.xpath_role_row})[1]{Locator.xpath_table_column}")
        del row_objects[0]
        rows_text = list()
