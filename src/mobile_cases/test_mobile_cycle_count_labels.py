...
@pytest.mark.acl
@pytest.mark.regression
def test_update_ohi_without_permission(mobile_api, permission_api, delete_shipto, delete_distributor_security_group):
...
@pytest.mark.regression
def test_update_ohi_with_disabled_track_ohi_option(mobile_api, delete_shipto):
...
@pytest.mark.regression
def test_update_ohi_with_qty_less_or_equal_min(mobile_api, conditions, delete_shipto):
    mobile_api.testrail_case_id = conditions["testrail_case_id"]
    mca = MobileCycleCountApi(mobile_api)
    la = LocationApi(mobile_api)
