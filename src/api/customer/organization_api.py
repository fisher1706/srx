...
class OrganizationApi(API):
    def create_site(self, dto):
        ...
        if response.status_code == 201:
            self.logger.info(f"New Site '{dto['number']}' has been successfully created")
    ...
    def create_subsite(self, dto):
        ...
        response = self.send_post(url, token, dto)
        if response.status_code == 201:
            self.logger.info(f"New Sub-Site '{dto['number']}' has been successfully created")
            response_json = response.json()
    ...
    def delete_site(self, site_id, expected_status_code=None):
        ...
        if response.status_code == 200:
            self.logger.info(Message.entity_with_id_operation_done.format(entity="Site", id=site_id, operation="deleted"))
        else:
            self.logger.info(Message.info_operation_with_expected_code.format(entity="Site", operation="deletion", status_code=response.status_code, content=response.content))
    ...
    def delete_subsite(self, subsite_id, expected_status_code=None):
        ...
        assert expected_status_code == response.status_code, Message.assert_status_code.format(expected=expected_status_code, actual=response.status_code, content=response.content)
        if response.status_code == 200:
            self.logger.info(Message.entity_with_id_operation_done.format(entity="Sub-Site", id=subsite_id, operation="deleted"))
        else:
            self.logger.info(Message.info_operation_with_expected_code.format(entity="Sub-Site", operation="deletion", status_code=response.status_code, content=response.content))
    ...
    def delete_supplier(self, supplier_id, expected_status_code=None):
        ...
        assert expected_status_code == response.status_code, Message.assert_status_code.format(expected=expected_status_code, actual=response.status_code, content=response.content)
        if response.status_code == 200:
            self.logger.info(Message.entity_with_id_operation_done.format(entity="Supplier", id=supplier_id, operation="deleted"))
        else:
            self.logger.info(Message.info_operation_with_expected_code.format(entity="supplier", operation="deletion", status_code=response.status_code, content=response.content))
