...
class SmartShelvesApi(API):
    def create_smart_shelf(self, dto):
        ...
        if response.status_code == 200:
            Log.info(Message.entity_operation_done.format(entity="Smart Shelf", operation="created"))
        else:
    ...
    def get_door_configuration(self, locker_id):
        ...
        if response.status_code == 200:
            Log.info(Message.entity_operation_done.format(entity="Locker Door configuration", operation="got"))
        else:
            Error.error(str(response.content))
    ...
    def get_smart_shelves_id(self, locker_name):
        ...
        if response.status_code == 200:
            Log.info(Message.entity_operation_done.format(entity="Smart Shelf ID", operation="got"))
        else:
            Error.error(str(response.content))
        response_json = response.json()
    ...
    def delete_smart_shelves(self, smart_shelves_id):
        ...
        for _ in range(1, 5):
            ...
            if response.status_code == 200:
                Log.info(Message.entity_with_id_operation_done.format(entity="Smart Shelf", id=smart_shelves_id, operation="deleted"))
                break
            elif response.status_code == 400:
                Log.info(f"Smart shelf with ID = '{smart_shelves_id}' cannot be deleted now")
    ...
    def update_smart_shelf(self, locker_body, serial_number=None, distributor_id=None, locker_body_second=None):
        ...
        if distributor_id is not None:
            ...
            if locker_body_second is None:
                Error.error("You need specify new locker")
        ...
        if response.status_code == 200:
            Log.info(Message.entity_with_id_operation_done.format(entity="Smart Shelf", id=smart_shelf_dto['id'], operation="updated"))
        else:
            Error.error(str(response.content))
    ...
    def get_smart_shelf(self, locker_name):
        ...
        response = self.send_get(url, token, params=params)
        if response.status_code == 200:
            Log.info(Message.entity_operation_done.format(entity="Smart Shelf", operation="got"))
        else:
            Error.error(str(response.content))
        response_json = response.json()
