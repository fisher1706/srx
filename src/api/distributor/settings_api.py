...
class SettingsApi(API):
    def update_reorder_controls_settings_shipto(self, dto, shipto_id):
        ...
        if response.status_code == 200:
            Log.info(Message.entity_with_id_operation_done.format(entity="Reorder Controls settings of shipto", id=shipto_id, operation="updated"))
        else:
    ...
    def get_reorder_controls_settings_for_shipto(self, shipto_id):
        ...
        if response.status_code == 200:
            Log.info(Message.entity_with_id_operation_done.format(entity="Checkout Software settings of shipto", id=shipto_id, operation="got"))
        else:
            Error.error(str(response.content))
        response_json = response.json()
        if not bool(response_json["data"]):
            Error.error(f"Checkout software settings of shipto with ID = '{shipto_id}' are empty")
    ...
    def update_autosubmit_settings_shipto(self, dto, shipto_id):
        ...
        response = self.send_post(url, token, dto)
        if response.status_code == 200:
            Log.info(Message.entity_with_id_operation_done.format(entity="Auto-Submit settings of shipto", id=shipto_id, operation="updated"))
        else:
            Error.error(str(response.content))
    ...
    def update_rl_rules_settings_shipto(self, dto, shipto_id):
        ...
        response = self.send_post(url, token, dto)
        if response.status_code == 200:
            Log.info(Message.entity_with_id_operation_done.format(entity="RL Rules settings of shipto", id=shipto_id, operation="updated"))
        else:
            Error.error(str(response.content))
    ...
    def update_serialization_settings_shipto(self, dto, shipto_id):
        ...
        response = self.send_post(url, token, dto)
        if response.status_code == 200:
            Log.info(Message.entity_with_id_operation_done.format(entity="Serialization settings of shipto", id=shipto_id, operation="updated"))
        else:
            Error.error(str(response.content))
    ...
    def update_adjustment_settings(self, dto, shipto_id, customer_id=None):
        ...
        if response.status_code == 200:
            Log.info(Message.entity_with_id_operation_done.format(entity="Adjustment settings of shipto", id=shipto_id, operation="updated"))
        else:
            Error.error(str(response.content))
    ...
    def get_adjustment_settings(self, shipto_id, customer_id=None):
        ...
        response = self.send_get(url, token)
        if response.status_code == 200:
            Log.info(Message.entity_with_id_operation_done.format(entity="Adjustment settings of shipto", id=shipto_id, operation="got"))
        else:
            Error.error(str(response.content))
        response_json = response.json()
        return response_json["data"]
    ...
    def update_vmi_settings(self, dto, shipto_id):
        ...
        response = self.send_post(url, token, dto)
        if response.status_code == 200:
            Log.info(Message.entity_with_id_operation_done.format(entity="VMI List settings of shipto", id=shipto_id, operation="updated"))
        else:
            Error.error(str(response.content))
    ...
    def update_checkout_settings(self, dto, shipto_id):
        ...
        response = self.send_post(url, token, dto)
        if response.status_code == 200:
            Log.info(Message.entity_with_id_operation_done.format(entity="QR Code Kit & Checkout Portal settings of shipto", id=shipto_id, operation="updated"))
        else:
            Error.error(str(response.content))
    ...
    def update_customer_level_catalog_flag(self, dto, customer_id=None):
        ...
        response = self.send_post(url, token, dto)
        if response.status_code == 200:
            Log.info(Message.entity_with_id_operation_done.format(entity="CLC flag of customer", id=customer_id, operation="updated"))
        else:
            Error.error(str(response.content))
    ...
    def get_cache_settings(self, shipto_id):
        ...
        response = self.send_get(url, token)
        if response.status_code == 200:
            Log.info(Message.entity_with_id_operation_done.format(entity="Cache settings of ShipTo", id=shipto_id, operation="got"))
        else:
            Error.error(str(response.content))
        response_json = response.json()
        return response_json["data"]
    ...
    def update_cache_settings(self, dto, shipto_id):
        ...
        response = self.send_post(url, token, dto)
        if response.status_code == 200:
            Log.info(Message.entity_with_id_operation_done.format(entity="Cache settings of ShipTo", id=shipto_id, operation="updated"))
        else:
            Error.error(str(response.content))
    ...
    def update_critical_min_and_stockout_alert_settings(self, dto, shipto_id):
        ...
        response = self.send_post(url, token, dto)
        if response.status_code == 200:
            Log.info(Message.entity_with_id_operation_done.format(entity="Critical Min & Stockout settings of ShipTo", id=shipto_id, operation="updated"))
        else:
            Error.error(str(response.content))
    ...
    def update_vmi_list_integration_settings(self, dto, shipto_id):
        ...
        response = self.send_post(url, token, dto)
        if response.status_code == 200:
            Log.info(Message.entity_with_id_operation_done.format(entity="VMI List integration settings of ShipTo", id=shipto_id, operation="updated"))
        else:
            Error.error(str(response.content))
    ...
    def sync_erp_connection_settings(self):
        ...
        if response.status_code == 200:
            Log.info(Message.entity_operation_done.format(entity="ERP Connection Settings", operation="synced"))
        else:
            Error.error(str(response.content))
    ...
    def update_reorder_list_submit_integration_settings(self, shipto_id):
        ...
        response = self.send_post(url, token, dto)
        if response.status_code == 200:
            Log.info(Message.entity_with_id_operation_done.format(entity="Reorder List Submit Integration settings of ShipTo", id=shipto_id, operation="updated"))
        else:
            Error.error(str(response.content))
