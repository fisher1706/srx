...
def total_triggers_include_all_movement_statuses(case):
    ...
    try:
        ...
        assert location_body[0]["inventoryStatus"] == "MOVING", f"New location should be created in MOVING inventory status, now {location_body[0]['inventoryStatus']}"
        ...
        location_body = la.get_location_by_sku(new_shipto, product_body["partSku"])
        assert location_body[0]["inventoryStatus"] == "SLOW", f"New location should be in SLOW inventory status, now {location_body[0]['inventoryStatus']}"
        ...
        location_body = la.get_location_by_sku(new_shipto, product_body["partSku"])
        assert location_body[0]["inventoryStatus"] == "FROZEN", f"New location should be in FROZEN inventory status, now {location_body[0]['inventoryStatus']}"
